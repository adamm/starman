...
static uint8_t sparkle_state[SPARKLE_MAX_LEDS];
...
void sparkle_step() {
    while(1) {
        for (uint8_t i = 0; i < SPARKLE_MAX_LEDS; i++) {
            ...
            memset(framebuffer.active, 0, DISPLAY_LIGHTS_TOTAL);
            ...
            if (sparkle_state[i] == 0) {
                sparkle_leds[i] = random_value_within_int(DISPLAY_LIGHTS_TOTAL);
            ...
            framebuffer.active[x][y] = sparkle_state[i];

            // Use sparkle_state to identify adjacent LEDs. Set their
            // brightness relative to the distance from the centre of the
            // sparkle.

            // FIXME: This is stupid inefficient -- figure out the algorithm :)
            if (sparkle_state[i] < 64) {
                // No adjacent LEDs are lit up
            }
            else if (sparkle_state[i] < 128) {
                // Directly adjacent LEDs are lit up
                framebuffer.active[x-1][y] = sparkle_state[i] - 64;
                framebuffer.active[x+1][y] = sparkle_state[i] - 64;
                framebuffer.active[x][y-1] = sparkle_state[i] - 64;
                framebuffer.active[x][y+1] = sparkle_state[i] - 64;
                framebuffer.active[x+1][y+1] = sparkle_state[i] - 128;
                framebuffer.active[x+1][y-1] = sparkle_state[i] - 128;
                framebuffer.active[x-1][y+1] = sparkle_state[i] - 128;
                framebuffer.active[x-1][y-1] = sparkle_state[i] - 128;
            }
            else if (sparkle_state[i] < 192) {
                // Two directly adjacent LEDs are lit up
                framebuffer.active[x-1][y] = sparkle_state[i] - 64;
                framebuffer.active[x+1][y] = sparkle_state[i] - 64;
                framebuffer.active[x][y-1] = sparkle_state[i] - 64;
                framebuffer.active[x][y+1] = sparkle_state[i] - 64;
                ...
                framebuffer.active[x-1][y+1] = sparkle_state[i] - 128;
                framebuffer.active[x-1][y-1] = sparkle_state[i] - 128;
                framebuffer.active[x+2][y+2] = sparkle_state[i] - 192;
                framebuffer.active[x+2][y-2] = sparkle_state[i] - 192;
                framebuffer.active[x-2][y+2] = sparkle_state[i] - 192;
                framebuffer.active[x-2][y-2] = sparkle_state[i] - 192;
            }
            else {
                // Three directly adjacent LEDs are lit up
                framebuffer.active[x-1][y] = sparkle_state[i] - 64;
                framebuffer.active[x+1][y] = sparkle_state[i] - 64;
                framebuffer.active[x][y-1] = sparkle_state[i] - 64;
                framebuffer.active[x][y+1] = sparkle_state[i] - 64;
                ...
                framebuffer.active[x-2][y+2] = sparkle_state[i] - 192;
                framebuffer.active[x-2][y-2] = sparkle_state[i] - 192;
                framebuffer.active[x+3][y+3] = sparkle_state[i] - 256;
                framebuffer.active[x+3][y-3] = sparkle_state[i] - 256;
                framebuffer.active[x-3][y+3] = sparkle_state[i] - 256;
                framebuffer.active[x-3][y-3] = sparkle_state[i] - 256;
            }
...
void sparkle_start(void) {
    ...
    for (uint8_t i = 0; i < SPARKLE_MAX_LEDS; i++) {
        ...
        sparkle_state[i] = random_value_within_int(SPARKLE_MAX_STATE);
        sparkle_direction[i] = random_value_within_int(1);
    }

    xTaskCreate(sparkle_step, "sparkle", 2048, NULL, 5, &sparkle_task);
}
